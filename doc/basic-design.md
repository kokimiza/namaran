...
4. 難易度の凡例（4匹の動物と段階、抽選の確率。要件定義 §7.1）
5. footer
...
* `/levels` — 4匹の動物（ハリネズミ・孔雀・バイソン・クジラ、要件定義 §7.1）と、それぞれの問題数、抽選の確率。`/topics` はタグの一覧だけにし、難易度や日付への案内は置かない
