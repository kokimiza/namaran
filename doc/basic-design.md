...
* `/level/{level}` — その難易度のドリルを新しい順に並べた1枚。難易度は全言語・全種別・全日付にまたがるので、タグより桁違いに速く伸びる（ハリネズミは1日に数行ずつ増える）。そこで読むためではなく**走査するための形**にする：月ごとに1つの表（見出しは `2026-09` と件数）、ページ先頭に各月へのページ内リンク、1ドリル1行で「日付・言語・種別・お題」、日付は同じ日の行の先頭に1度だけ（`rowspan`）、リンクはお題だけ。列幅は固定で、月の表どうしの列がそろう。狭い画面では各行を「言語 種別」とお題の2行に畳む。番号つきのページ送りはしない——月がそのままページの区切りになり、URLもsitemapも増えない。1ページが重くなりすぎたら、年ごとに分けるのが次の手である。動物はどの行も同じなので見出しに1度だけ出す。4枚は常にすべて生成する（まだ1問も無い難易度でも空の一覧を置く）。難易度の集合は固定なので、`tag/` と違って消えるページは無い。同じ `level/` にある `.svg` は手で描いたもので、スクリプトは触らない
...
* 解説の脚注（§7.1）は、本文との間を block 段（2rem）あけ、`--panel` を背景へ半分だけ寄せた薄い色の角丸の面に置く。先頭に「Notes」の小さなラベル（CSSの `::before`。マークアップは増やさない）。本文より一段小さく、色は `--ink-muted`。番号（本文側の `sup.fn-ref` と `ol` のマーカー）は等幅でアクセント色にする。上付き文字は `vertical-align` ではなく `position` で持ち上げ、その行の行間を広げない
